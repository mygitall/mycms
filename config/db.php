...
/**
 * 清理过期限流文件（概率触发，减少 IO 开销）
 */
function cleanExpiredRateLimitFiles() {
    // 1% 概率触发清理
    if (mt_rand(1, 100) !== 1) return;

    $dir = getAppRuntimeDir();
    $files = @glob($dir . '/*');
    if (!is_array($files)) return;

    $now = time();
    $maxAge = 86400; // 保留最多 24 小时
    $count = 0;

    foreach ($files as $f) {
        $bn = basename($f);
        // 仅清理限流相关文件
        if (!preg_match('/^(ratelimit_|globalratelimit_)/', $bn)) continue;
        $mtime = @filemtime($f);
    ...
    if ($count > 0) {
        error_log('ratelimit cleanup: removed ' . $count . ' expired files');
    }
}
