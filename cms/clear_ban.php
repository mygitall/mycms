...
if (file_exists($banEnabledFile)) {
    ...
    if (isset($cfg['enabled'])) $prevEnabled = (bool)$cfg['enabled'];
}
file_put_contents($banEnabledFile, json_encode(['enabled' => false, 'updated_at' => date('Y-m-d H:i:s')]));
$results['switch'] = $prevEnabled ? "已关闭封禁开关（需手动在后台重新开启）" : "封禁开关原本已是关闭状态";
?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
...
<div class="card">
  ...
  <div class="note">
    <strong>提示：</strong>IP 封禁开关已自动关闭。进入后台后请在顶部工具栏重新开启 IP 封禁功能，以保护后台安全。关闭后如果再次被封禁，可重新访问本页面解除。
  </div>
</div>
</body>
</html>
