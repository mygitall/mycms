...
function saveSearchHistory($pdo, $keyword, $userId = null) {
    ...
    $check = $pdo->prepare("SELECT id, search_count FROM `{$prefix}search_history` WHERE keyword = :kw AND ((user_id = :uid) OR (user_id IS NULL AND :uid2 IS NULL)) ORDER BY id DESC LIMIT 1");
    $check->execute([':kw' => $keyword, ':uid' => $uid, ':uid2' => $uid]);
...
$params     = [];
$paramIdx   = 0;
$whereParamKeys = [];  // 记录哪些 key 是 WHERE 参数
...
foreach ($activeSources as $type => $cfg) {
    ...
    // 构建带 relevance 的子查询（所有分支必须有相同列数）
    $selectExpr = "SELECT '{$type}' AS `type`, {$colList}, " . ($relevanceExpr !== '' ? "({$relevanceExpr})" : "NULL") . " AS `relevance`";
...
try {
    ...
    // 仅绑定 WHERE 参数（count 查询不使用 relevance 额外参数）
    foreach ($whereParamKeys as $k) {
        if (isset($params[$k])) {
            $cntStmt->bindValue($k, $params[$k]);
        }
    }
    $cntStmt->execute();
    $total = 0;
